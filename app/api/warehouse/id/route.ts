...
export const DELETE = withAuth(async (request: NextRequest & { user: any }, { params }: { params: { id: string } }) => {
  try {
    ...
    if (warehouse._count.inventoryItems > 0) {
      return NextResponse.json({ error: "Cannot delete warehouse with inventory items" }, { status: 400 })
    }

    if (warehouse._count.users > 0) {
      return NextResponse.json({ error: "Cannot delete warehouse with assigned users" }, { status: 400 })
    }
